...
void ccViewer::selectEntity(ccHObject* toSelect)
{
	...
	if (toSelect)
	{
		...
		m_actionClipBox->setEnabled(isCloud);
...
{
	ccHObject* currentRoot = m_glWindow->getSceneDB();
	if (currentRoot) { m_selectedObject = nullptr; m_selectedEntities.clear(); m_glWindow->setSceneDB(nullptr); m_glWindow->redraw(); delete currentRoot; currentRoot = nullptr; }
		...
		if (newGroup)
		{
			if (!options.normalsDisplayedByDefault)
			{
				...
				for (ccHObject* cloud : clouds)
					if (cloud) static_cast<ccGenericPointCloud*>(cloud)->showNormals(false);
			}
			...
			if (!scaleAlreadyDisplayed)
			{
				for (unsigned i = 0; i < newGroup->getChildrenNumber(); ++i)
				{
					...
					if (ent->isA(CC_TYPES::POINT_CLOUD))
					{
						...
						if (pc->hasScalarFields()) { pc->setCurrentDisplayedScalarField(0); pc->showSFColorsScale(true); scaleAlreadyDisplayed = true; }
					}
					else if (ent->isKindOf(CC_TYPES::MESH))
					{
						...
						{
							mesh->showSF(true); scaleAlreadyDisplayed = true;
							static_cast<ccPointCloud*>(mesh->getAssociatedCloud())->showSFColorsScale(true);
						}
					}
				}
			}
			if (!firstLoadedEntity) firstLoadedEntity = newGroup;
		}
		if (result == CC_FERR_CANCELED_BY_USER) break;
	}
...
void ccViewer::doActionComputeNormals()
{
	...
	{
		// Parameters are gathered; computation requires caller-side infrastructure
		ccLog::Print("[Normals] Parameters configured (computation requires octree and more infrastructure)");
		m_glWindow->redraw();
	}
}
...
void ccViewer::doActionFilterByValue()
{
	...
	{
		// Read filter range from dialog's UI widgets (the dialog inherits from Ui::FilterByValueDialog)
		ccLog::Print(QString("[Filter by Value] Mode: %1").arg(dlg.mode() == ccFilterByValueDlg::SPLIT ? "SPLIT" : "EXPORT"));
	}
}
...
void ccViewer::doActionSORFilter()
{
	...
	if (dlg.exec() == QDialog::Accepted)
	{
		...
		ccLog::Print(QString("[SOR Filter] KNN=%1, nSigma=%2 (computation requires full implementation)").arg(knn).arg(nSigma));
	}
}
...
// ============================================================================
// SLIM: Tools menu - new implementations
// ============================================================================
...
QString ccViewer::pythonScriptsDir() const
{
	...
	// Fallback: use the directory where the .m files are
	pythonDir = exeDir + "/../../python";
	if (QDir(pythonDir).exists())
		return QDir(pythonDir).absolutePath();
...
QString ccViewer::runPythonScript(const QString& scriptName, const QStringList& args)
{
	...
	{
		ccLog::Error("Python scripts directory not found");
		return QString();
	}
	...
	QString pythonExe = "C:/Users/30892/AppData/Local/Programs/Python/Python313/python.exe";
	...
	if (!m_pythonProcess->waitForFinished(30000)) // 30 second timeout
	{
	...
	{
		QString err = QString::fromUtf8(m_pythonProcess->readAllStandardError());
		ccLog::Error(QString("Python script error: %1").arg(err));
		return QString();
	}

	return QString::fromUtf8(m_pythonProcess->readAllStandardOutput());
}
...
void ccViewer::doActionClipBox()
{
	...
	if (activate)
	{
		...
		// Size to cloud bounding box with some padding
		ccBBox cloudBB = cloud->getOwnBB();
		CCVector3 diag = cloudBB.getDiagVec();
		CCVector3 padding = diag * static_cast<PointCoordinateType>(0.05);
		ccBBox paddedBB(
		...
		m_clipBox->addAssociatedEntity(m_selectedObject);
		m_clipBox->setVisible(true);
		m_clipBox->showBox(true);
		m_clipBox->setEnabled(true);

		// Add to scene
		ccHObject* root = m_glWindow->getSceneDB();
		if (root)
		{
			root->addChild(m_clipBox);
		}

		m_clipBoxActive = true;
		m_glWindow->redraw();

		ccLog::Print("[Clip Box] Drag the box handles to adjust the clipping region. Uncheck 'Clip by Box' to apply and remove.");
	}
	else
	{
		...
		if (m_clipBox && m_clipBoxActive)
		{
			...
			msgBox.setInformativeText(tr("'Yes' = crop and keep inside points\n'No' = crop and keep outside points\n'Cancel' = discard clipping"));
			...
			{
				// Cancel - remove clip box without applying
				removeFromDB(m_clipBox, false);
			...
			if (cloud)
			{
				...
				// Modify visibility for keepInside/keepOutside
				unsigned insideCount = 0;
				for (unsigned i = 0; i < cloud->size(); ++i)
				{
					bool isInside = (visTable[i] == CCCoreLib::POINT_VISIBLE);
					if (keepInside)
						visTable[i] = isInside ? CCCoreLib::POINT_VISIBLE : CCCoreLib::POINT_HIDDEN;
					else
						visTable[i] = isInside ? CCCoreLib::POINT_HIDDEN : CCCoreLib::POINT_VISIBLE;
					if (visTable[i] == CCCoreLib::POINT_VISIBLE)
						++insideCount;
				}

				if (insideCount > 0 && insideCount < cloud->size())
				{
					...
					if (newCloud)
					{
						...
						addToDB(newCloud, true, true, false, true);
						ccLog::Print(QString("[Clip Box] Created cropped cloud: %1 points").arg(newCloud->size()));
					}
				}
				else
				{
					ccLog::Warning(QString("[Clip Box] No points in the %1 region").arg(keepInside ? "inside" : "outside"));
				}
			}

			// Remove clip box from scene
			removeFromDB(m_clipBox, false);
			m_clipBox->releaseAssociatedEntities();
			m_clipBoxActive = false;
			m_glWindow->redraw();
...
{
	if (!m_clipBox || !m_clipBoxActive) return;
...
void ccViewer::doActionTLSPlane()
{
	...
	QTemporaryFile tempFile("tls_input_XXXXXX.csv");
	...
	if (doc.isNull())
	{
		ccLog::Error("Failed to parse Python output");
		return;
	}
...
void ccViewer::doActionPointProjection()
{
	...
	// Export cloud to temp CSV
	QTemporaryFile tempFile("projection_input_XXXXXX.csv");
	if (!exportSelectedCloudToCSV(tempFile))
	{
		ccLog::Error("Failed to export point cloud");
		return;
	...
	QTemporaryFile tempOutFile("projection_output_XXXXXX.csv");
	tempOutFile.open();
	...
	if (createCloud && QFile::exists(outPath))
	{
		...
		{
			// Copy colors and SF from original
			if (cloud->hasColors())
			{
				projectedCloud->reserveTheRGBTable();
...
void ccViewer::doActionKNNSearch()
{
	...
	// Export cloud to temp CSV
	QTemporaryFile tempFile("knn_input_XXXXXX.csv");
	if (!exportSelectedCloudToCSV(tempFile))
	{
		ccLog::Error("Failed to export point cloud");
		return;
