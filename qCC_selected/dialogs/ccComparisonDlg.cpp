...
// Local
#include "ccCommon.h"
// #include "ccHistogramWindow.h" -- SLIM: disabled
// // #include "mainwindow.h" -- SLIM
#include <ccMainAppInterface.h>  // SLIM: removed, using ccMainAppInterface instead
...
void ccComparisonDlg::updateDisplay(bool showSF, bool showRef)
{
	...
	// SLIM: Replaced MainWindow static calls with parent interface
	{
		ccMainAppInterface* app = dynamic_cast<ccMainAppInterface*>(parentWidget());
		if (app) { app->updateUI(); app->refreshAll(false); }
	}
}
