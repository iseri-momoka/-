...
LasScalarFieldLoader::handleScalarField(LasScalarField& sfInfo, ccPointCloud& pointCloud, T currentValue)
{
	if (!sfInfo.sf)
	{
		...
		// If there are already points in the cloud, fill the new scalar field
		// with default values up to the current size using resizeSafe (batch operation, much faster than per-point addElement)
		if (pointCloud.size() > 0)
		{
			if (!newSf->resizeSafe(pointCloud.size(), true, static_cast<ScalarType>(T{})))
			{
				return CC_FERR_NOT_ENOUGH_MEMORY;
			}
		}
