                                    ...
                                    LoadParameters& parameters)
{
	...
	// Use UTF-8 encoding for file path
	// laszip's LASfopen() on Windows converts UTF-8 → UTF-16 → uses _wfopen()
	QByteArray utf8FileName = fileName.toUtf8();
	bool openOk = (laszip_open_reader(laszipReader, utf8FileName.constData(), &isCompressed) == 0);

#ifdef Q_OS_WIN
	if (!openOk)
	{
		// Fallback: try local 8-bit encoding (GBK on Chinese Windows)
		QByteArray localFileName = fileName.toLocal8Bit();
		openOk = (laszip_open_reader(laszipReader, localFileName.constData(), &isCompressed) == 0);
	}
#endif
	...
	if (!openOk)
	{
		...
		ccLog::Warning("[LAS] Failed to open: '%s'", qUtf8Printable(fileName));
	...
	// Pre-allocate RGB table upfront if format supports RGB (avoids O(N) backfill)
	bool hasRGB = LasDetails::HasRGB(laszipHeader->point_data_format);
	if (hasRGB)
	{
	...
	for (auto interval : chunksToRead)
	{
		...
		for (unsigned i = 0; i < intervalRef.pointCount; ++i)
		{
			...
			// Periodically update UI to prevent freezing during large file loads
			if (i % 100000 == 0)
			{
